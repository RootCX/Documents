...
export function ShareModal({ documentId, onClose }: Props) {
  ...
  async function refresh() {
    ...
    try {
      ...
      const match = all.find(s => (s.context as { document_id?: string })?.document_id === documentId);
  ...
  async function enable() {
    setWorking(true);
    try {
      const share = await client.createPublicShare(APP_ID, { context: { document_id: documentId } });
        ...
        {loading ? (
            ...
            <div className="flex items-start gap-2 text-sm">
              ...
              <div>
                ...
                <div className="text-muted-foreground text-xs">
                  Anyone with the link can read this document. Created {new Date(active.createdAt).toLocaleDateString()}.
          ...
          <div className="space-y-4">
            <div className="flex items-start gap-2 text-sm">
              ...
              <div>
                <div className="font-medium">This document is private</div>
                <div className="text-muted-foreground text-xs">
                  Make it public to get a read-only link anyone can open without signing in.
                </div>
              </div>
            </div>
